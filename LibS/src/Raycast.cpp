template <class T>
Raycast<T>::Raycast(const Ray<T>& _ray, const std::vector<typename Raycast<T>::Hit>& _hits) : ray(_ray), hits(_hits)
{

}
template <class T>
Raycast<T>::Raycast(const Ray<T>& _ray, std::vector<typename Raycast<T>::Hit>&& _hits) : ray(_ray), hits(std::move(_hits))
{
